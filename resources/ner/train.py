...
def evaluate(dev_loader, model, mode='dev'):
    ...
    with torch.no_grad():
        for idx, batch_samples in enumerate(dev_loader):
            ...
            # compute model output and loss
            out = model((batch_data, batch_token_starts),
...
# 返回提取后的结果
##
def extract_result(text, labelsId):
    """extract_result"""
    predict_tag = []
    for sen in labelsId:
        temp = []
        for i in sen:
            temp.append(config.id2label[i])
    ...
    for i, label in enumerate(predict_tag):
        ...
        if label.startswith(u"B-"):
            is_start += 1
            if is_start == 2:
                entities.append((text[start_idx:end_idx], label[2:], start_idx, end_idx, "置信度"))
                is_start = 0
            start_idx = i
